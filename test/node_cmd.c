...
EC_TEST_MAIN()
{
	...
	testres |= EC_TEST_CHECK_PARSE(node, 4, "command", "subset3", "subset2", "1");
	testres |= EC_TEST_CHECK_PARSE(node, 5, "command", "subset2", "subset3", "subset1", "1");
	testres |= EC_TEST_CHECK_PARSE(
		node, 6, "command", "subset3", "subset1", "subset4", "subset2", "4"
	);
	...
	node = EC_NODE_CMD(
		EC_NO_ID, "good morning [count] bob|bobby|michael", ec_node_int("count", 0, 10, 10)
	);
	if (node == NULL) {
		EC_LOG(EC_LOG_ERR, "cannot create node\n");
		return -1;
	...
	testres |= EC_TEST_CHECK_COMPLETE(node, "", EC_VA_END, "good", EC_VA_END);
	testres |= EC_TEST_CHECK_COMPLETE(node, "g", EC_VA_END, "good", EC_VA_END);
	testres |= EC_TEST_CHECK_COMPLETE(
		node, "good", "morning", "", EC_VA_END, "bob", "bobby", "michael", EC_VA_END
	);
